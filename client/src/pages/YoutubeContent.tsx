...
export default function YoutubeContent() {
  ...
  useEffect(() => {
    ...
    const colors = [
      new Color(0xFF0000), // YouTube red
      new Color(0xCC0000),
      new Color(0xFF4444),
    ]
  ...
  const handleSaveToNotes = (content: string) => {
    ...
    const newNote = {
      id: Date.now().toString(),
      title: `Script YouTube - ${new Date().toLocaleDateString('fr-FR')}`,
      content,
      platform: 'youtube',
              ...
              <div className="relative z-10 flex-1 overflow-y-auto mb-4 space-y-4">
                {messages.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-500">Commencez à discuter pour générer des scripts YouTube</p>
                  </div>
                ) : (
