...
export default function YoutubeHub() {
  ...
  // Three.js background with YouTube red theme
  useEffect(() => {
    ...
    curves.forEach((curve, index) => {
      ...
      const material = new ShaderMaterial({
        uniforms: {
          time: { value: 0 },
          color: { value: new Color(0xff0000) }, // YouTube red
        },
        vertexShader: `
          uniform float time;
          varying vec2 vUv;
      ...
      <div className="fixed top-8 left-0 right-0 flex justify-center z-50">
        <div className="bg-white/80 backdrop-blur-md border border-slate-200/60 rounded-full px-6 py-3 shadow-lg">
          <div className="flex items-center gap-6">
            ...
            <span className="text-slate-800 font-medium">YouTube</span>
            <button
              onClick={() => {
                logout()
                setLocation('/')
            ...
            <div className="relative w-[95vw] max-w-[1400px] h-full">
              <div className="relative backdrop-blur-xl bg-white/30 border border-slate-200/60 rounded-3xl p-8 shadow-2xl h-full overflow-hidden flex flex-col">
                ...
                <div className="relative z-10 flex-1 overflow-auto">
                  ...
                  >
                    <h2 className="text-3xl font-light text-slate-800 mb-2">YouTube</h2>
                    <p className="text-slate-500 text-sm">Gérez vos contenus et idées pour YouTube</p>
                  </motion.div>
                  ...
                  >
                    ...
                    <BentoCard
                      ...
                      className="col-span-1 w-[340px] h-[260px]"
                      background={
                        <div className="w-full h-full bg-gradient-to-br from-blue-400/20 via-blue-500/20 to-blue-600/20" />
                      }
                      Icon={() => <ContenusIcon />}
                      description="Organisez et gérez vos contenus YouTube"
                      href="/contenu/youtube/contenus"
                      cta="Ouvrir"
                    />
                  </motion.div>
                </div>
              ...
              <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-red-200/20 to-red-300/20 blur-xl scale-110 -z-10" />
            </div>
          </div>
        </div>
      </motion.main>
