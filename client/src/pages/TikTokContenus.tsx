...
export default function YoutubeContenus() {
  ...
  const [contents, setContents] = useState<TikTokContent[]>(() => {
    const saved = localStorage.getItem('persom_tiktok_contents')
    return saved ? JSON.parse(saved) : []
  })

  const [showModal, setShowModal] = useState(false)
  const [editingContent, setEditingContent] = useState<TikTokContent | null>(null)
  ...
  useEffect(() => {
    ...
    curves.forEach((curve, index) => {
      ...
      const material = new ShaderMaterial({
        uniforms: {
          time: { value: 0 },
          color: { value: new Color(0x00f2ea) },
        },
        vertexShader: `
          uniform float time;
          varying vec2 vUv;
  ...
  useEffect(() => {
    localStorage.setItem('persom_tiktok_contents', JSON.stringify(contents))
  }, [contents])
  ...
  const handleOpenModal = (content?: TikTokContent) => {
    if (content) {
      ...
      <div className="fixed top-8 left-0 right-0 flex justify-center z-50">
        <div className="bg-white/80 backdrop-blur-md border border-slate-200/60 rounded-full px-6 py-3 shadow-lg">
          <div className="flex items-center gap-6">
            ...
            <span className="text-slate-800 font-medium">Mes contenus TikTok</span>
            <button
              onClick={() => {
                logout()
                setLocation('/')
            ...
            <div className="relative w-[95vw] max-w-[1400px] h-full">
              <div className="relative backdrop-blur-xl bg-white/30 border border-slate-200/60 rounded-3xl p-8 shadow-2xl h-full overflow-hidden flex flex-col">
                ...
                <div className="relative z-10 flex-1 overflow-auto">
                  ...
                  >
                    <div>
                      <h2 className="text-3xl font-light text-slate-800 mb-2">Mes contenus TikTok</h2>
                      <p className="text-slate-500 text-sm">Organisez et gérez vos contenus TikTok</p>
                    </div>
                    <button
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-semibold text-slate-800">
                {editingContent ? 'Modifier le contenu' : 'Nouveau contenu TikTok'}
              </h3>
              <button
                onClick={() => {
                  setShowModal(false)
